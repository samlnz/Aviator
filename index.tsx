...
export default function WebGLStarter() {
	...
	React.useEffect(() => {
		// Fix: Safely call send or sendMessage depending on react-unity-webgl version (v8 vs v9+)
		if (myUnityContext) {
			const sendMsg = myUnityContext.sendMessage || myUnityContext.send;
			if (typeof sendMsg === 'function') {
				sendMsg.call(myUnityContext, "GameManager", "RequestToken", JSON.stringify({
					gameState: flag
				}));
			}
		}
	...
	return (
		<div className="crash-container">
			<div className="canvas">
				{/* Fix: In react-unity-webgl v9+, the prop 'unityContext' was replaced by 'unityProvider'. 
				    The provider is either the context itself or a property of it depending on the version used. */}
				<Unity unityProvider={myUnityContext?.unityProvider || myUnityContext} matchWebGLToCanvasSize={true} />
			</div>
